...
public class ShoutView: UIView {
  ...
  public private(set) lazy var backgroundView: UIView = {
    ...
    let color1 = view.backgroundColor!.CGColor as CGColorRef
    if view.backgroundColor!.respondsToSelector(#selector(UIColor.getHue(_:saturation:brightness:alpha:))) {
        view.backgroundColor!.getHue(&h, saturation: &s, brightness: &b, alpha: &a)
    }
    let color2 = UIColor(hue: h, saturation: s, brightness: b * 1.2, alpha: a).CGColor as CGColorRef
    let color3 = UIColor(hue: h, saturation: s, brightness: b * 1.5, alpha: a).CGColor as CGColorRef
    let color4 = UIColor.lightGrayColor().CGColor as CGColorRef
